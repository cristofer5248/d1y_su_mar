...
public interface IClienteDao extends PagingAndSortingRepository<Cliente, Long> {
	...
	@Query(value = "select c from Cliente c join fetch c.usuario u left join c.giro g where c.id>=?1 order by c.id asc", countQuery = "select count(c) from Cliente c join c.usuario u join c.giro g where c.id>=?1 order by c.id asc")
