...
//	@Query(value = "select i from Inventario i where i.codigoProveedor=?1 and i.estado=true LIMIT 1")
	...
	@Query(value = "select i from Inventario i join fetch i.producto p join fetch p.proveedor prov join fetch i.movimientos m where i.codigoProveedor like %?1%  group by m  order by i.id desc", countQuery = "select count(i) from Inventario i join i.producto p join p.proveedor prov join i.movimientos m where i.codigoProveedor like %?1% group by m order by i.id desc ")
